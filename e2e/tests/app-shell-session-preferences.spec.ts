...
test.describe('App shell and preferences', () => {
  test('desktop navigation activates and renders every route', async ({ page }) => {
    ...
    const destinations = [
      ...
      { path: '/metabolism', name: 'Metabolism', heading: 'Metabolism & Recovery' },
  ...
  test('dark mode preference persists across reload', async ({ page }) => {
    await registerFreshUser(page);
    const toggleButton = page.locator('header button[title="Switch to Dark Mode"]').first();
    ...
    await expect(lightSaved.json()).resolves.toMatchObject({ dark_mode: false });
  ...
  test.describe('mobile navigation', () => {
    ...
    test('bottom navigation reaches every tab without horizontal overflow', async ({ page }) => {
      await registerFreshUser(page);
      const mobileNavigation = page.locator('nav.md\\:hidden');
