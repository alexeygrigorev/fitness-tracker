...
import { HttpError, ValidationFailure } from '../types.js';
...
interface MealValues {
  ...
  notes: string | null;
  source: MealSource;
  food_items: NestedFoodItemRecord[] | undefined;
}
...
function decimal(
  ...
  if (options.maximumDigits !== undefined) {
    const placesLimit = options.maximumDigits === 5
      ? DECIMAL_LIMITS.small
      : DECIMAL_LIMITS.standard;
    if (Math.abs(parsed) > placesLimit) {
      addError(
        errors,
        field,
        `Ensure that there are no more than ${
          options.maximumDigits === 5 ? 2 : 2
        } decimal places.`,
      );
    }
  }
...
function integer(
  ...
  field: string,
  value: unknown,
  options: { allowNull?: boolean; choices?: readonly number[] } = {},
): number | null | undefined {
  if (value === null && options.allowNull) {
    return null;
  }
...
function validateFood(
  ...
  for (const field of numericFields) {
    if (isCreate || field in data) {
      ...
      if (value !== undefined) {
        parsedNumbers[field] = field === 'insulinResponse'
          ? (value === null ? null : decimal([], '', value, { maximumDigits: 5 }) ?? null)
          : value;
      }
    }
  }
  parsedNumbers.insulinResponse = currentNumbers.insulinResponse;
  if ('insulinResponse' in data) {
    const value = decimal(errors, 'insulinResponse', data.insulinResponse, {
      allowNull: true,
      maximumDigits: 5,
    });
    if (value !== undefined) {
      parsedNumbers.insulinResponse = value;
    }
  }
...
function validateMeal(
  ...
  if ('loggedAt' in data && data.loggedAt !== null) {
    const parsed = new Date(String(data.loggedAt));
...
async function foodMap(
  ...
  const foods = await context.repository.batchGet<FoodItemRecord>(
    uniqueIds.map((id) => ({ pk: `FOOD#${id}`, sk: 'METADATA' })),
  );
...
function mealResponse(
  ...
  return {
    ...
    notes: meal.notes,
    source: meal.source,
    food_items: nestedResponse(meal.food_items),
    ...mealTotals(meal.food_items, foods),
  };
}

function templateResponse(template: MealTemplateRecord): Record<string, unknown> {
  return {
    id: template.id,
    name: template.name,
    category: template.category,
    notes: template.notes,
    food_items: nestedResponse(template.food_items),
  };
}
...
async function resolveNestedFoods(
  ...
  const foods = await foodMap(context, items.map((item) => item.foodId));
  const errors: JsonObject = {};
  ...
  for (const [index, item] of items.entries()) {
    ...
    const id = await context.repository.nextId('meal_food_item');
    resolved.push({
      id,
      food_id: item.foodId,
      grams: item.grams,
